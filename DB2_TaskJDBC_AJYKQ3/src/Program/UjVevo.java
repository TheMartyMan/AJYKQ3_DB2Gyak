...
public class UjVevo extends JDialog {
	...
	public UjVevo(JFrame f) {
		...
		}
		{
			...
			lblCim.setBounds(10, 118, 122, 26);
		...
		}
		{
			...
			btnFelvesz.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					else if(!filledTF(szülido)) uzenet("A születési idõ mezõ nem lehet üres!");
					else if(!datumEll(szülido)) uzenet("A születési idõ formátuma helytelen!");
					else {
					dbm.addVevo(RTF(vevoid), RTF(nev), RTF(tel), RTF(cim), RTF(szülido));
					dispose();
					}
				}
			});
		...
		{
			szülido = new JTextField();
			szülido.setColumns(10);
			szülido.setBounds(142, 154, 107, 25);
